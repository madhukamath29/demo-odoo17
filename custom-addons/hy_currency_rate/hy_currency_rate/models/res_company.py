...
class ResCompany(models.Model):
    ...
    def update_freight_currency_rates(self):
        currency = self.env["freight.currency"]
        currency_rate = self.env["freight.currency.rate"]
        all_currencies = currency.search([])
    ...
    def run_update_currency(self):
        ...
        if to_update:
            ...
            to_update.update_freight_currency_rates()

    @api.model
    def get_rate(self, curr1, curr2, date):
        url = "https://www.customs.govt.nz/news/datafeeds/exchange"
        ...
        if response.status_code == 200:
            ...
            print("currrr1111"+curr1)
            print("curr2"+curr2)

            for rate_data in exchange_rates:
                if  rate_data["currencyCode"] == curr2.upper():
                    return rate_data["rateNow"]
